...
RSpec.describe FatherlyAdvice::JsonWebToken::Client, :env_change do
  ...
  context 'with non-prefixed env vars' do
    ...
    context 'with all needed' do
      let(:env) do
        {
          'OAUTH2_TOKEN_URL' => 'https://auth.acimacredit.com/api/oauth/token',
          'OAUTH2_M2M_CLIENT_ID' => 'some_client_id',
          'OAUTH2_M2M_CLIENT_SECRET' => 'some_client_secret',
      ...
      context 'apps' do
        ...
        context 'bank_account' do
          ...
          context 'provider' do
            ...
            it('client_id') { expect(provider.client_id).to eq 'some_client_id' }
            it('client_secret') { expect(provider.client_secret).to eq 'some_client_secret' }
          end
          context 'client' do
            ...
            context 'can_get_access_tokens?' do
              let(:result) { described_class.can_get_access_tokens? name }
              it('returns true') { expect(result).to eq true }
            end
          end
        end
      end
    end
    context 'with some missing' do
        ...
        context 'apps' do
          ...
          context 'bank_account' do
            ...
            context 'provider' do
              ...
              it('name') { expect(provider.name).to eq 'auth0' }
              it('prefix') { expect(provider.prefix).to eq '' }
              it('token_url') { expect(provider.token_url).to be_nil }
              it('jwks_url') { expect(provider.jwks_url).to be_nil }
              it('user_info_url') { expect(provider.user_info_url).to be_nil }
              it('authorize_url') { expect(provider.authorize_url).to be_nil }
              it('issuer') { expect(provider.issuer).to be_nil }
            ...
            context 'client' do
              context 'get_access_tokens' do
                let(:result) { described_class.get_access_tokens name }
                context 'with failure' do
                  it 'on missing path' do
                    described_class.clear_cache

                    Timecop.travel(auth0_token_generation_time) do
                      expect(Excon).to_not receive(:post)
                      expect { result }.to raise_error ArgumentError, 'missing path'
                    end
                  end
                end
              end
              context 'can_get_access_tokens?' do
                let(:result) { described_class.can_get_access_tokens? name }
                it('returns false') { expect(result).to eq false }
              end
            end
          end
        end
      end
      context 'OAUTH2_M2M_CLIENT_ID' do
        let(:env) do
          {
            'OAUTH2_TOKEN_URL' => 'https://auth.acimacredit.com/api/oauth/token',
            # 'OAUTH2_M2M_CLIENT_ID' => 'some_client_id',
          ...
          context 'bank_account' do
            ...
            context 'client' do
              context 'get_access_tokens' do
                let(:result) { described_class.get_access_tokens name }
                context 'with failure' do
                  it 'on missing client_id' do
                    described_class.clear_cache

                    Timecop.travel(auth0_token_generation_time) do
                      expect(Excon).to_not receive(:post)
                      expect { result }.to raise_error ArgumentError, 'missing client_id'
                    end
                  end
                end
              end
    ...
    let(:env) do
      {
        ...
        'OAUTH2_AUTH0_M2M_CLIENT_ID' => 'some_client_id',
        'OAUTH2_AUTH0_M2M_CLIENT_SECRET' => 'some_client_secret',
